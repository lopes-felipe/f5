...
export function parseWhenAst(expression: string): KeybindingWhenNode | null {
...
export const parseKeybindingWhenExpression = parseWhenAst;
...
export function compileResolvedKeybindingRule(rule: KeybindingRule): ResolvedKeybindingRule | null {
  ...
  if (rule.when !== undefined) {
    const whenAst = parseWhenAst(rule.when);
...
export function shortcutSignature(shortcut: KeybindingShortcut): string {
  return [
    shortcut.modKey ? "1" : "0",
...
export function shortcutEquals(left: KeybindingShortcut, right: KeybindingShortcut): boolean {
...
export function keybindingShortcutContext(rule: KeybindingRule): string | null {
  ...
  const whenAst = rule.when ? parseWhenAst(rule.when) : undefined;
  const whenCanonical = whenAst ? encodeWhenAst(whenAst) : "";
  return `${encoded}\u0000${whenCanonical}`;
}
...
export function canonicalizeKeybindingTarget(rule: KeybindingRule): {
  readonly command: KeybindingRule["command"];
  readonly shortcut: KeybindingShortcut;
  readonly whenCanonical: string | null;
} | null {
  const shortcut = parseKeybindingShortcut(rule.key);
  if (!shortcut) return null;
  const whenAst = rule.when ? parseWhenAst(rule.when) : undefined;
  ...
  return {
    command: rule.command,
    shortcut,
    whenCanonical: whenAst ? encodeWhenAst(whenAst) : null,
  };
}

export function matchesKeybindingTarget(stored: KeybindingRule, target: KeybindingRule): boolean {
  if (stored.command !== target.command) return false;
  const storedCanonical = canonicalizeKeybindingTarget(stored);
  const targetCanonical = canonicalizeKeybindingTarget(target);
  if (!storedCanonical || !targetCanonical) return false;
  return (
    shortcutEquals(storedCanonical.shortcut, targetCanonical.shortcut) &&
    storedCanonical.whenCanonical === targetCanonical.whenCanonical
  );
}

export function hasEquivalentKeybindingRule(
  entries: ReadonlyArray<KeybindingRule>,
  rule: KeybindingRule,
): boolean {
  return entries.some((entry) => matchesKeybindingTarget(entry, rule));
}
