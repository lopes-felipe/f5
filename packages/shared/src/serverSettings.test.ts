...
describe("serverSettings helpers", () => {
  ...
  it("preserves redacted OpenCode provider instance passwords on unrelated edits", () => {
    const opencodeId = ProviderInstanceId.make("opencode_personal");
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
      providerInstances: {
        [opencodeId]: {
    ...
    expect(
      ...
      }).providerInstances[opencodeId]?.config,
    ).toEqual({
      binaryPath: "opencode",
      serverPassword: "secret-password",
    });
  ...
  it("allows OpenCode provider instance passwords to be cleared by omitting the key", () => {
    const opencodeId = ProviderInstanceId.make("opencode_personal");
    const current = {
      ...DEFAULT_SERVER_SETTINGS,
      providerInstances: {
