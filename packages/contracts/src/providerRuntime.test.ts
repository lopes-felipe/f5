...
describe("ProviderRuntimeEvent", () => {
  ...
  it("decodes compaction.recommended for automatic compaction triggers", () => {
    const parsed = decodeRuntimeEvent({
    ...
    expect(parsed.type).toBe("compaction.recommended");
    if (parsed.type !== "compaction.recommended") {
      throw new Error("expected compaction.recommended");
    }
    expect(parsed.payload.estimatedTokens).toBe(165000);
    expect(parsed.payload.thresholdTokens).toBe(160000);
  });
