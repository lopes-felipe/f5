...
it.layer(NodeServices.layer)("effect-acp client", (it) => {
  ...
  it.effect("includes only a bounded stderr tail when a child exits during startup", () =>
    Effect.gen(function* () {
      const handle = yield* makeHandle({
        ACP_MOCK_EXIT_IMMEDIATELY_CODE: "7",
        ACP_MOCK_EXIT_STDERR: "x".repeat(6000) + "\nAuthentication failed\n",
      });
      ...
      const outcome = yield* Effect.gen(function* () {
        ...
        return yield* acp.agent.initialize({ protocolVersion: 1, clientCapabilities: {} });
      }).pipe(Effect.provide(context), Effect.exit, Effect.ensuring(Scope.close(scope, Exit.void)));
      ...
      if (Schema.is(AcpError.AcpTransportError)(error)) {
        assert.include(error.message, "ACP process exited with code 7");
        assert.include(error.message, "Authentication failed");
        assert.isAtMost(error.message.length, 4300);
        assert.notInclude(error.message, "x".repeat(5000));
      }
    }),
  );

  for (const alias of [false, true]) {
    it.effect(`initializes, prompts and handles requests with SDK elicitation alias=${alias}`, () =>
      Effect.gen(function* () {
        ...
        const handle = yield* makeHandle({ ACP_MOCK_ELICITATION_ALIAS: alias ? "1" : "0" });
        const scope = yield* Scope.make();
