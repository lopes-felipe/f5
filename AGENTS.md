...
- `bun run upstream-ports:check` must pass for upstream-port work. The single ledger’s pinned first-parent
...
- `apps/web/src/appSettings.ts` is the authoritative schema and persistence path for web-local settings.
- `packages/contracts/src/settings.ts` provides shared typing/defaults only; do not add web persistence fields to `ClientSettingsSchema`.
- Settings routing in `apps/web/src/hooks/useSettings.ts` must stay derived from the schema field keys. Never add a hand-maintained client/server key allowlist.
