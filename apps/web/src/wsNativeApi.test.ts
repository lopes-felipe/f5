...
describe("wsNativeApi", () => {
  ...
  it("forwards harness validation requests to the websocket server method", async () => {
    ...
    const api = createWsNativeApi();
    await api.server.validateHarnesses({
      providerOptions: {
        codex: {
    ...
    expect(requestMock).toHaveBeenCalledWith(WS_METHODS.serverValidateHarnesses, {
      providerOptions: {
        codex: {
          binaryPath: "/tmp/codex",
          homePath: "/tmp/.codex",
  ...
  it("forwards thread file-change summary requests to the orchestration websocket method", async () => {
    ...
    const api = createWsNativeApi();
    await api.orchestration.getThreadFileChanges({
      threadId: ThreadId.makeUnsafe("thread-1"),
      afterSequenceExclusive: 5,
    });

    expect(requestMock).toHaveBeenCalledWith(ORCHESTRATION_WS_METHODS.getThreadFileChanges, {
      threadId: "thread-1",
      afterSequenceExclusive: 5,
    });
  });

  it("forwards single file-change transcript requests to the orchestration websocket method", async () => {
    requestMock.mockResolvedValue({ fileChange: null });
    const { createWsNativeApi } = await import("./wsNativeApi");

    const api = createWsNativeApi();
    await api.orchestration.getThreadFileChange({
      threadId: ThreadId.makeUnsafe("thread-1"),
      fileChangeId: OrchestrationFileChangeId.makeUnsafe("filechange:thread-1:item-1"),
    });

    expect(requestMock).toHaveBeenCalledWith(ORCHESTRATION_WS_METHODS.getThreadFileChange, {
      threadId: "thread-1",
