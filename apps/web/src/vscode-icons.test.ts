...
describe("getVscodeIconUrlForEntry", () => {
  ...
  it("handles Windows-style separators when deriving basenames", () => {
    assert.equal(basenameOfPath("C:\\repo\\apps\\web\\src\\main.tsx"), "main.tsx");
    assert.isTrue(
      getVscodeIconUrlForEntry("C:\\repo\\pnpm-workspace.yaml", "file", "dark").endsWith(
        "/file_type_pnpm.svg",
      ),
    );
  });
