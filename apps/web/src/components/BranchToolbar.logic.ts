...
/**
 * The combobox popup switches from a plain `ComboboxList` + `ScrollArea` to
 * the virtualized `LegendList` path once the filtered branch list gets
 * large enough that rendering every row is wasteful. 40 is the threshold
 * upstream settled on during the `@legendapp/list` migration.
 */
export const BRANCH_LIST_VIRTUALIZATION_THRESHOLD = 40;

export function shouldVirtualizeBranchListForCount(count: number): boolean {
  return count > BRANCH_LIST_VIRTUALIZATION_THRESHOLD;
}

/**
 * Upstream `96c9306d` swapped unconditional mouse-hover scroll-into-view for
 * a keyboard-only guard, so the popup no longer jumps when the pointer
 * happens to pass over an off-screen item. The guard mirrors the behavior
 * inside `BranchToolbarBranchSelector`'s `onItemHighlighted` handler.
 */
export function shouldScrollHighlightedBranchIntoView(input: {
  isMenuOpen: boolean;
  highlightedIndex: number;
  highlightReason: string;
}): boolean {
  const { isMenuOpen, highlightedIndex, highlightReason } = input;
  return isMenuOpen && highlightedIndex >= 0 && highlightReason === "keyboard";
}
