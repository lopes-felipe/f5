...
export function supportsClaudeTraitsControls(model: string | null | undefined): boolean {
  return (
    getReasoningEffortOptions("claudeAgent", model).length > 0 ||
    supportsClaudeThinkingToggle(model) ||
    supportsClaudeFastMode(model)
  );
}
...
function ClaudeTraitsMenuContentImpl(props: {
  ...
  const defaultReasoningEffort = getDefaultReasoningEffort("claudeAgent", props.model);
  ...
  const setClaudeModelOptions = (nextClaudeModelOptions: ClaudeModelOptions | undefined) => {
    ...
    // Route the options change through recordModelSelection so the recent-model
    // MRU used by `model.switchRecent` keeps the options slice attached to the
    // model it belongs to. Fall back to the stored last-selected claude model
    // when the composer draft hasn't explicitly chosen one yet.
    const currentModel =
      (typeof props.model === "string" && props.model.length > 0 ? props.model : null) ??
      draft.model ??
      useModelPreferencesStore.getState().lastModelByProvider.claudeAgent ??
      null;
    if (currentModel) {
      recordModelSelection("claudeAgent", currentModel, nextProviderModelOptions);
    } else {
      useModelPreferencesStore
        .getState()
        .setLastModelOptions("claudeAgent", nextProviderModelOptions);
    }
  };

  if (!supportsClaudeTraitsControls(props.model)) {
    return null;
  }
...
export const ClaudeTraitsPicker = memo(function ClaudeTraitsPicker(props: {
  ...
  const { effort, thinkingEnabled, fastModeEnabled, ultrathinkPromptControlled, supportsFastMode } =
    getSelectedClaudeTraits(props.model, prompt, modelOptions);
  const supportsTraitControls = supportsClaudeTraitsControls(props.model);
  if (!supportsTraitControls) {
    return null;
  }
  const triggerLabel = [
    ultrathinkPromptControlled
