...
it("signs in to the selected Enterprise host", async () => {
  await render(<GithubAccountPanel />);
  ...
  await page.getByRole("button", { name: "Sign in with GitHub" }).click();
  expect(api.githubLoginStart).toHaveBeenCalledWith({ host: "ghe.example.com" });
});

it("falls back to the token form when gh is missing and clears the submitted token", async () => {
  status = {
    available: false,
    state: "idle",
    error: "Install GitHub CLI (gh) to sign in with your browser, or use a personal access token.",
  };
  await render(<GithubAccountPanel />);
  await expect.element(page.getByText(/Install GitHub CLI \(gh\)/)).toBeVisible();
  await expect.element(page.getByRole("button", { name: "Sign in with GitHub" })).toBeDisabled();
  // The token form starts open without gh.
...
it("does not claim gh is missing before the server answers", async () => {
  vi.mocked(api.githubLoginStatus).mockImplementation(() => new Promise(() => {}));
  await render(<GithubAccountPanel />);
  await expect.element(page.getByRole("button", { name: "Sign in with GitHub" })).toBeEnabled();
  await expect.element(page.getByText(/Install GitHub CLI/)).not.toBeInTheDocument();
});
