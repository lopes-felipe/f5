...
function Sidebar({
  ...
  const resolvedResizable = React.useMemo<SidebarResolvedResizableOptions | null>(() => {
    ...
    return {
      defaultWidth:
        typeof options.defaultWidth === "number" && Number.isFinite(options.defaultWidth)
...
function SidebarRail({
  ...
  const handleDoubleClick = React.useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      if (resolvedResizable.defaultWidth === null) return;
        ...
        resolvedResizable.shouldAcceptWidth?.({
          currentWidth:
            sidebarRoot
              ...
              ?.getBoundingClientRect().width ?? nextWidth,
          nextWidth,
          rail: event.currentTarget,
          side: sidebarInstance?.side ?? "left",
          sidebarRoot,
...
function SidebarMenuSkeleton({
  ...
  // Random width between 50 to 90% unless a deterministic width is supplied.
