...
function ComposerSurroundSelectionPlugin() {
  ...
  useEffect(() => {
    // We rely on the browser's `beforeinput` event rather than KEY_DOWN_COMMAND
    // so that compose / non-ASCII pairs (e.g. «») and IME-driven input are
    // handled consistently — KEY_DOWN is fired before the IME resolves the
    // final character. beforeinput gives us the finalized data.
    const handleBeforeInput = (event: InputEvent) => {
      if (event.inputType !== "insertText") return;
      const data = event.data;
      if (!data || data.length !== 1) return;
      const pair = COMPOSER_SURROUND_PAIRS[data];
      if (!pair) return;
      ...
      editor.update(() => {
        ...
        // Bail if the selection crosses or touches an inline-token node: the
        // composer's cursor math (getAbsoluteOffsetForPoint,
        // ComposerInlineTokenBackspacePlugin, etc.) assumes TextNode content
        // and would be destabilized by splitting around a token.
        const selectionNodes = selection.getNodes();
        if (selectionNodes.some((node) => isComposerInlineTokenNode(node))) {
          return;
        }

        const anchorNode = selection.anchor.getNode();
        const focusNode = selection.focus.getNode();
        const anchorExpanded = getExpandedAbsoluteOffsetForPoint(
          anchorNode,
        ...
        // A single editor.update() call produces one history entry, so undo
        // collapses the wrap operation back to the original selection.
        selection.insertText(`${open}${selectedText}${close}`);
        handled = true;
      });
    ...
    return editor.registerRootListener((nextRoot, prevRoot) => {
      if (prevRoot) {
        prevRoot.removeEventListener("beforeinput", handleBeforeInput);
      }
      if (nextRoot) {
        nextRoot.addEventListener("beforeinput", handleBeforeInput);
      }
    });
  }, [editor]);
