...
import { appendAttachedFilesToPrompt } from "./attachedFiles";
import {
...
describe("terminalContext", () => {
  ...
  it("derives attached file paths after stripping the trailing hidden block", () => {
    const prompt = appendAttachedFilesToPrompt(
      appendTerminalContextsToPrompt("Investigate this", [makeContext()]),
      ["/repo/src/file with spaces.ts", "/repo/@special/file.md"],
    );
    ...
    expect(deriveDisplayedUserMessageState(prompt)).toEqual({
      ...
      copyText: [
        ...
        "",
        "Attached files:",
        "- /repo/src/file with spaces.ts",
        "- /repo/@special/file.md",
      ].join("\n"),
      contextCount: 1,
      previewTitle: "Terminal 1 lines 12-13\n12 | git status\n13 | On branch main",
      contexts: [
      ...
      attachedFilePaths: ["/repo/src/file with spaces.ts", "/repo/@special/file.md"],
    });
  });

  it("hides malformed attached-file blocks from the visible message text", () => {
    const prompt = [
      appendTerminalContextsToPrompt("Investigate this", [makeContext()]),
      "",
      "<attached_files>",
  ...
  it("does not copy raw hidden metadata blocks for attached files", () => {
    const prompt = appendAttachedFilesToPrompt("Inspect this", ["/outside/repo/notes.md"]);
    const displayed = deriveDisplayedUserMessageState(prompt);

    expect(displayed.copyText).toBe(
      ["Inspect this", "", "Attached files:", "- /outside/repo/notes.md"].join("\n"),
    );
    expect(displayed.copyText).not.toContain("<attached_files>");
  });
});
