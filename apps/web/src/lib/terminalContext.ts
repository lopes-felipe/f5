...
import { extractTrailingAttachedFiles } from "./attachedFiles";
...
export function deriveDisplayedUserMessageState(prompt: string): DisplayedUserMessageState {
  const extractedAttachedFiles = extractTrailingAttachedFiles(prompt);
  const extractedContexts = extractTrailingTerminalContexts(extractedAttachedFiles.promptText);
  const copyText = buildDisplayedUserMessageCopyText({
    visibleText: extractedContexts.promptText,
    contexts: extractedContexts.contexts,
    attachedFilePaths: extractedAttachedFiles.filePaths,
  });
  return {
    visibleText: extractedContexts.promptText,
    copyText,
...
function buildDisplayedUserMessageCopyText(input: {
  visibleText: string;
  contexts: ReadonlyArray<ParsedTerminalContextEntry>;
  attachedFilePaths: ReadonlyArray<string>;
}): string {
  ...
  if (input.contexts.length > 0) {
    sections.push(
      [
        "Terminal contexts:",
        ...input.contexts.map(({ header, body }) =>
          body.length > 0 ? `${header}\n${body}` : header,
        ),
      ].join("\n\n"),
    );
  }
  if (input.attachedFilePaths.length > 0) {
    sections.push(
      ["Attached files:", ...input.attachedFilePaths.map((filePath) => `- ${filePath}`)].join("\n"),
    );
  }
