...
import type { PrHubComparisonIdentity, PrHubReviewDraft } from "@t3tools/contracts";
...
export function prepareReviewOperation(
  ...
  input: {
    ...
    quick?: { comparison: PrHubComparisonIdentity; body: string };
  },
) {
  return Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
      ...
      Effect.gen(function* () {
        ...
        if (existing) {
          if (
            ...
            (input.quick
              ? existing.payload.source !== "quick_review" ||
                existing.payload.draft.content.body !== input.quick.body ||
                !prComparisonsEqual(existing.payload.draft.comparison, input.quick.comparison)
              : existing.payload.source === "quick_review")
          )
        ...
        const draft: PrHubReviewDraft | null = input.quick
          ? {
              ...
              comparison: input.quick.comparison,
              content: { body: input.quick.body, comments: [], viewedFiles: [] },
            }
          : yield* readPrHubReviewDraft(owner);
        ...
        const payload = {
          version: 2 as const,
          source: input.quick ? ("quick_review" as const) : ("editor_review" as const),
          draft,
          event: input.event,
        ...
        VALUES (${owner.provider}, ${owner.host}, ${owner.viewerId}, ${owner.repo}, ${owner.number}, ${input.id},
          'review', 'prepared', ${hash}, ${payloadJson}, ${input.quick ? null : draft.version}, ${nonce}, ${now}, ${now})`;
        if (!input.quick)
  ...
  return Effect.gen(function* () {
    ...
    return yield* sql.withTransaction(
      Effect.gen(function* () {
        ...
        if (
          rows[0].draft_version !== null &&
          ["succeeded", "failed_before_send", "rejected", "abandoned"].includes(input.to)
        ) {
