...
import { Effect, FileSystem, Layer, Metric, PlatformError, Scope } from "effect";
...
it.layer(TestLayer)("git integration", (it) => {
  ...
  it.effect("records observability metrics for git command execution", () =>
    Effect.gen(function* () {
      ...
      const value = yield* core.readConfigValue("/tmp/repo", "user.email");
