...
const DEFAULT_CLAUDE_MODEL_SELECTION = createModelSelection(
...
const resolveInputModelSelection = (selection: ModelSelection | undefined, model?: string) =>
  selection ??
  createModelSelection(
    DEFAULT_CLAUDE_MODEL_SELECTION.instanceId,
    model ?? DEFAULT_CLAUDE_MODEL_SELECTION.model,
  );
...
export const makeClaudeTextGeneration = Effect.fn("makeClaudeTextGeneration")(function* (
  ...
  const generateStructuredJson: TextGenerationShape["generateStructuredJson"] = Effect.fn(
    "ClaudeTextGeneration.generateStructuredJson",
  )(function* (input) {
