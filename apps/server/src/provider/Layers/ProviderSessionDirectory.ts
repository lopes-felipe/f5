...
const makeProviderSessionDirectory = Effect.gen(function* () {
  ...
  const listBindingsByProject: ProviderSessionDirectoryShape["listBindingsByProject"] = (
    projectId,
  ) =>
    repository.listByProjectId({ projectId }).pipe(
      Effect.mapError(
        toPersistenceError("ProviderSessionDirectory.listBindingsByProject:listByProjectId"),
      ),
      Effect.flatMap((rows) =>
        Effect.forEach(rows, (row) =>
