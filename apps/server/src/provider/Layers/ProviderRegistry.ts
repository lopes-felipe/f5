...
export const ProviderRegistryLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const providerStatusCacheDir =
      config.providerStatusCacheDir ?? path.join(config.stateDir, "provider-status-cache");
    ...
    // Boot-only: seed `providersRef` from the on-disk per-instance cache
    // before the first live sync. This intentionally avoids calling
    // `getSnapshot` here; `syncLiveSources` performs the initial refresh
    // once below.
