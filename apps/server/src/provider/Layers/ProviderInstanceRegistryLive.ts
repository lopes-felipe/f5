...
import { ServiceMap, Effect, Equal, Exit, Layer, PubSub, Ref, Schema, Scope, Stream } from "effect";
    ...
    .update(
      JSON.stringify(
        sortForStableJson({
          ...
          config: fingerprintableProviderConfig(input.entry.driver, input.decodedConfig),
        }),
      ),
    )
    .digest("hex");
  ...
  Effect.gen(function* () {
    ...
    const createResult = yield* driver
      .create({
        ...
        enabled:
          entry.enabled !== undefined ? entry.enabled : (decodedConfigEnabled(typedConfig) ?? true),
        config: typedConfig,
      })
