...
import { ServiceMap, Deferred, Effect, Fiber, Layer, Schema, Stream } from "effect";
...
// tests assumed.
const makeResolveCursorSettings: Effect.Effect<
  Effect.Effect<CursorSettings, never, never>,
  never,
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  it.effect("fails a turn containing only a transport diagnostic", () =>
    Effect.gen(function* () {
      ...
      if (completed?.type === "turn.completed") {
        ...
        assert.equal(completed.payload.errorMessage, diagnostic);
      }
  ...
  it.effect("starts a session and maps mock ACP prompt flow to runtime events", () =>
    Effect.gen(function* () {
      ...
      if (configured?.type === "session.configured") {
        assert.deepInclude(configured.payload.config, {
          model: "default",
  ...
  it.effect("broadcasts runtime events to multiple stream consumers", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["session.started", "session.configured", "session.state.changed", "thread.started"],
      );
      assert.deepStrictEqual(
  ...
  it.effect("switches model in-session via session/set_config_option", () =>
    Effect.gen(function* () {
      ...
      const lastConfigured = configuredEvents[configuredEvents.length - 1];
      assert.equal(lastConfigured?.type, "session.configured");
      if (lastConfigured?.type === "session.configured") {
        assert.deepInclude(lastConfigured.payload.config, {
          model: "composer-2",
          fast_mode_state: "on",
          session_id: "mock-session-1",
        });
  ...
  it.effect("applies Cursor modelOptions when modelSelection has no option payload", () =>
    Effect.gen(function* () {
      const adapter = yield* CursorAdapter;
      const serverSettings = yield* ServerSettingsService;
      const threadId = ThreadId.make("cursor-model-options");
      const tempDir = yield* Effect.promise(() => mkdtemp(path.join(os.tmpdir(), "cursor-acp-")));
      const requestLogPath = path.join(tempDir, "requests.ndjson");
      const argvLogPath = path.join(tempDir, "argv.txt");
      yield* Effect.promise(() => writeFile(requestLogPath, "", "utf8"));
      ...
      assert.isAbove(fastConfigRequests.length, 0, "should apply cursor fast mode");
      const lastFastConfig = fastConfigRequests[fastConfigRequests.length - 1];
      assert.equal((lastFastConfig?.params as Record<string, unknown>)?.value, "true");
