...
export function buildGrokBackgroundTaskEvents(input: {
  ...
  const start = (
    ...
    events.push({
      type: "task.started",
      payload: { taskId: task.payload.taskId, taskType, description },
      ...attribution(task),
    });
    return task;
  };
  const complete = (
    ...
    events.push({
      type: "task.completed",
      payload: { taskId: task.payload.taskId, status, ...(summary ? { summary } : {}) },
      ...attribution(task),
    });
  };
      ...
      if (status === "running") {
        events.push({
          ...
          payload: {
            ...
            description: summary ?? task.payload.description,
          },
          ...attribution(task),
        });
      } else {
