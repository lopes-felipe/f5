    ...
    expect(update(output)).toEqual([
      {
        type: "task.started",
        turnId,
        payload: { taskId: id, taskType, description },
      },
    ]);
    expect(update(output)).toEqual([]);
    ...
    expect(events).toEqual([
      {
        ...
        payload:
          type === "task.progress"
            ? { taskId: "monitor-1", description: "result" }
            : { taskId: "monitor-1", status: expectedStatus, summary: "result" },
      },
    ]);
  ...
  it("starts unknown poll tasks before progress or completion and ignores subagents", () => {
    ...
    expect(
      events
        ...
        .map((event) => event.payload.taskType),
    ).toEqual(["shell", "monitor"]);
