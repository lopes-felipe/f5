...
describe("AcpRuntimeModel", () => {
  ...
  describe("decideToolCallUpdateEmission", () => {
    ...
    it("preserves a background-task start without printable detail", () => {
      expect(
        decideToolCallUpdateEmission({
          previous: toolCall(undefined, "inProgress"),
          next: {
            ...toolCall(undefined, "inProgress"),
            data: { rawOutput: { type: "BackgroundTaskStarted", taskId: "task" } },
          },
