...
export default Effect.gen(function* () {
  ...
  if (!existingColumns.has("scope_key")) {
    const skippedRows = yield* sql<{ readonly count: number }>`
      SELECT COUNT(*) AS count
      FROM project_mcp_configs
      WHERE project_id IS NULL
    `;

    if ((skippedRows[0]?.count ?? 0) > 0) {
      yield* Effect.logWarning(
        "skipping project_mcp_configs rows with null project_id during scope migration",
        {
          skippedRows: skippedRows[0]?.count ?? 0,
        },
      );
    }

    yield* sql`
      DROP TABLE IF EXISTS project_mcp_configs_next
    `;
