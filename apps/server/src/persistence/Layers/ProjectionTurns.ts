...
const makeProjectionTurnRepository = Effect.gen(function* () {
  ...
  const getLatestRunningProjectionTurnByThreadId = SqlSchema.findOneOption({
    Request: ListProjectionTurnsByThreadInput,
    Result: ProjectionTurnByIdDbRowSchema,
    execute: ({ threadId }) =>
      sql`
  ...
  const getLatestRunningByThreadId: ProjectionTurnRepositoryShape["getLatestRunningByThreadId"] = (
    input,
  ) =>
    getLatestRunningProjectionTurnByThreadId(input).pipe(
      Effect.mapError(
        toPersistenceSqlOrDecodeError(
          "ProjectionTurnRepository.getLatestRunningByThreadId:query",
          "ProjectionTurnRepository.getLatestRunningByThreadId:decodeRow",
        ),
      ),
      Effect.flatMap((rowOption) =>
        Option.match(rowOption, {
