...
function redactProviderInstanceConfig(instance: ProviderInstanceConfig): ProviderInstanceConfig {
  if (
    ...
    !("serverPassword" in instance.config)
  ) {
    return instance;
  }
  return {
...
export function redactServerSettingsForClient(settings: ServerSettings): ServerSettings {
  const providerInstances = Object.fromEntries(
    Object.entries(settings.providerInstances).map(([instanceId, instance]) => {
      const redactedInstance = instance.environment
        ? {
            ...instance,
            environment: instance.environment.map(redactProviderEnvironmentVariable),
          }
        : instance;
      return [instanceId, redactProviderInstanceConfig(redactedInstance)];
    }),
  );
  return {
    ...settings,
...
const makeServerSettings = Effect.gen(function* () {
  ...
  const settingsPath =
    serverConfig.settingsPath ?? pathService.join(serverConfig.stateDir, "settings.json");
