...
it.layer(NodeServices.layer)("resolveEditorLaunch", (it) => {
  ...
  it.effect("applies editor-specific navigation arguments", () =>
    Effect.gen(function* () {
      ...
      const windowsIdeaLineAndColumn = yield* resolveEditorLaunch(
        { cwd: "C:\\workspace\\src\\open.ts:71:5", editor: "idea" },
        "win32",
      );
      assert.deepEqual(windowsIdeaLineAndColumn, {
        command: "idea",
        args: ["--line", "71", "--column", "5", "C:\\workspace\\src\\open.ts"],
      });
    }),
  );

  it.effect(
    "uses platform-specific IntelliJ launcher fallbacks when bare idea is unavailable",
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const dir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-open-test-" });
        yield* fs.writeFileString(path.join(dir, "idea64.exe"), "MZ");
    ...
    Effect.gen(function* () {
      ...
      try {
        ...
        assert.equal(
          snapshots.some(
            (snapshot) =>
              snapshot.id === "t3_editor_launch_total" &&
              snapshot.attributes?.editorId === "cursor" &&
        ...
        assert.equal(
          snapshots.some(
            (snapshot) =>
              snapshot.id === "t3_editor_launch_total" &&
              snapshot.attributes?.editorId === "cursor" &&
              snapshot.attributes?.outcome === "unavailable-command",
          ),
          true,
        );
      } finally {
...
it.layer(NodeServices.layer)("resolveAvailableEditors", (it) => {
  it.effect("returns installed editors for command launches", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(editors, ["trae", "vscode-insiders", "vscodium", "idea", "file-manager"]);
    }),
  );
});
