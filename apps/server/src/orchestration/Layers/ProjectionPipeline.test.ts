...
projectionLayer("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const eventStore = yield* OrchestrationEventStore;
        const sql = yield* SqlClient.SqlClient;
        const threadId = ThreadId.makeUnsafe("thread-token-usage");
        const projectId = ProjectId.makeUnsafe("project-token-usage");
        const createdAt = "2026-04-10T10:00:00.000Z";
        const sessionAt = "2026-04-10T10:00:05.000Z";
        const compactedAt = "2026-04-10T10:00:10.000Z";
        const revertedAt = "2026-04-10T10:00:15.000Z";

        yield* eventStore.append({
          type: "thread.created",
          eventId: EventId.makeUnsafe("evt-token-created"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: createdAt,
          commandId: CommandId.makeUnsafe("cmd-token-created"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-token-created"),
          metadata: {},
          payload: {
            threadId,
            projectId,
      ...
      `;
        assert.deepEqual(rows, [
          { estimatedContextTokens: null, modelContextWindowTokens: 1_050_000 },
        ]);

        yield* eventStore.append({
          type: "thread.session-set",
          eventId: EventId.makeUnsafe("evt-token-session"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: sessionAt,
          commandId: CommandId.makeUnsafe("cmd-token-session"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-token-session"),
          metadata: {},
          payload: {
            threadId,
            session: {
      ...
      `;
        assert.deepEqual(rows, [
          { estimatedContextTokens: 48_000, modelContextWindowTokens: 1_050_000 },
        ]);

        yield* eventStore.append({
          type: "thread.compacted",
          eventId: EventId.makeUnsafe("evt-token-compacted"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: compactedAt,
          commandId: CommandId.makeUnsafe("cmd-token-compacted"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-token-compacted"),
          metadata: {},
          payload: {
            threadId,
            compaction: {
      ...
      `;
        assert.deepEqual(rows, [
          { estimatedContextTokens: 1_200, modelContextWindowTokens: 1_050_000 },
        ]);

        yield* eventStore.append({
          type: "thread.reverted",
          eventId: EventId.makeUnsafe("evt-token-reverted"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: revertedAt,
          commandId: CommandId.makeUnsafe("cmd-token-reverted"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-token-reverted"),
          metadata: {},
          payload: {
            threadId,
            turnCount: 0,
  ...
  it.effect("estimates token usage locally on thread.message-sent and persists the source", () =>
    Effect.gen(function* () {
      ...
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const threadId = ThreadId.makeUnsafe("thread-local-token-usage");
      const projectId = ProjectId.makeUnsafe("project-local-token-usage");
      const createdAt = "2026-04-10T11:00:00.000Z";
      const sessionAt = "2026-04-10T11:00:05.000Z";
      const messageAt = "2026-04-10T11:00:10.000Z";

      yield* eventStore.append({
        type: "thread.created",
        eventId: EventId.makeUnsafe("evt-local-created"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: createdAt,
        commandId: CommandId.makeUnsafe("cmd-local-created"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-local-created"),
        metadata: {},
        payload: {
          threadId,
          projectId,
          title: "Local token thread",
          model: "gpt-5.4",
          runtimeMode: "full-access",
          branch: null,
          worktreePath: null,
      ...
      yield* eventStore.append({
        type: "thread.session-set",
        eventId: EventId.makeUnsafe("evt-local-session"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: sessionAt,
        commandId: CommandId.makeUnsafe("cmd-local-session"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-local-session"),
        metadata: {},
        payload: {
          threadId,
          session: {
      ...
      yield* eventStore.append({
        type: "thread.message-sent",
        eventId: EventId.makeUnsafe("evt-local-message"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: messageAt,
        commandId: CommandId.makeUnsafe("cmd-local-message"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-local-message"),
        metadata: {},
        payload: {
          threadId,
          messageId: MessageId.makeUnsafe("message-local-1"),
          role: "user",
          text: "Hello there",
      ...
      const threadRows = yield* sql<{
        ...
        SELECT estimated_context_tokens AS "estimatedContextTokens"
        FROM projection_threads
        WHERE thread_id = ${threadId}
      `;
      ...
      assert.deepEqual(threadRows, [
        { estimatedContextTokens: roughTokenEstimateFromCharacters("Hello there".length) },
      ]);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const eventStore = yield* OrchestrationEventStore;
        const sql = yield* SqlClient.SqlClient;
        const threadId = ThreadId.makeUnsafe("thread-hidden-context-baseline");
        const projectId = ProjectId.makeUnsafe("project-hidden-context-baseline");
        const createdAt = "2026-04-10T12:00:00.000Z";
        const sessionAt = "2026-04-10T12:00:05.000Z";
        const messageAt = "2026-04-10T12:00:10.000Z";

        yield* eventStore.append({
          type: "thread.created",
          eventId: EventId.makeUnsafe("evt-hidden-baseline-created"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: createdAt,
          commandId: CommandId.makeUnsafe("cmd-hidden-baseline-created"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-hidden-baseline-created"),
          metadata: {},
          payload: {
            threadId,
            projectId,
            title: "Hidden baseline thread",
            model: "gpt-5.4",
            runtimeMode: "full-access",
            branch: null,
            worktreePath: null,
        ...
        yield* eventStore.append({
          type: "thread.session-set",
          eventId: EventId.makeUnsafe("evt-hidden-baseline-session"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: sessionAt,
          commandId: CommandId.makeUnsafe("cmd-hidden-baseline-session"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-hidden-baseline-session"),
          metadata: {},
          payload: {
            threadId,
            session: {
        ...
        yield* eventStore.append({
          type: "thread.message-sent",
          eventId: EventId.makeUnsafe("evt-hidden-baseline-message"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: messageAt,
          commandId: CommandId.makeUnsafe("cmd-hidden-baseline-message"),
          causationEventId: null,
          correlationId: CorrelationId.makeUnsafe("cmd-hidden-baseline-message"),
          metadata: {},
          payload: {
            threadId,
            messageId: MessageId.makeUnsafe("message-hidden-baseline-1"),
            role: "user",
            text: "Hello there",
            reasoningText: undefined,
            attachments: undefined,
      ...
      `;
        ...
        assert.deepEqual(threadRows, [
          {
            estimatedContextTokens: 1_500 + roughTokenEstimateFromCharacters("Hello there".length),
          },
        ]);
        assert.deepEqual(sessionRows, [
          {
...
it.effect("repairs an explicitly settled turn after a transient session error", () =>
  Effect.gen(function* () {
    const projectionPipeline = yield* OrchestrationProjectionPipeline;
    const eventStore = yield* OrchestrationEventStore;
    const sql = yield* SqlClient.SqlClient;
    ...
    const messageId = MessageId.makeUnsafe("assistant-terminal-session");
    const requestedAt = "2026-02-26T15:00:00.000Z";
    const runningAt = "2026-02-26T15:00:01.000Z";
    const messageAt = "2026-02-26T15:00:02.000Z";
    const errorAt = "2026-02-26T15:00:02.500Z";
    const readyAt = "2026-02-26T15:00:03.000Z";
    ...
    yield* appendAndProject({
      ...
      payload: {
        threadId,
        messageId: MessageId.makeUnsafe("user-terminal-session"),
        runtimeMode: "full-access",
        createdAt: requestedAt,
      },
    });
    ...
    yield* appendAndProject({
      type: "thread.session-set",
      eventId: EventId.makeUnsafe("evt-terminal-session-error"),
      aggregateKind: "thread",
      aggregateId: threadId,
      occurredAt: errorAt,
      commandId: CommandId.makeUnsafe("cmd-terminal-session-error"),
      causationEventId: null,
      correlationId: CorrelationId.makeUnsafe("cmd-terminal-session-error"),
      metadata: {},
      payload: {
        threadId,
        session: {
          ...
          runtimeMode: "full-access",
          activeTurnId: turnId,
          lastError: "Received invalid JSON from codex app-server.",
          updatedAt: errorAt,
        },
      },
    });

    const erroredRows = yield* sql<{
      readonly state: string;
      readonly completedAt: string | null;
    }>`
        ...
        WHERE thread_id = ${threadId}
          AND turn_id = ${turnId}
      `;
    assert.deepEqual(erroredRows, [{ state: "error", completedAt: errorAt }]);
    ...
    yield* appendAndProject({
      ...
      aggregateId: threadId,
      occurredAt: readyAt,
      commandId: CommandId.makeUnsafe("cmd-terminal-session-6"),
      causationEventId: null,
      correlationId: CorrelationId.makeUnsafe("cmd-terminal-session-6"),
      metadata: {},
      payload: {
        threadId,
