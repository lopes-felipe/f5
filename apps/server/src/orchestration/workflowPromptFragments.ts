...
- Verify claims about the codebase by reading every cited file and relevant adjacent flow; cite verified code evidence as \`file_path:line_number\`. Treat a plan that materially misdescribes current behavior as a critical problem even when its intent is right.
...
- Focus the report on actionable findings and cite \`file_path:line_number\` for every code-specific finding.
