...
function extractPromptField(prompt: string, field: string): string | undefined {
  const match = new RegExp(`Set "${field}" to "([^"]+)"`).exec(prompt);
  return match?.[1];
}

function buildOneOffPromptResponse(prompt: string): string {
  if (prompt.includes("strict JSON only")) {
    const updatedAt = extractPromptField(prompt, "updatedAt") ?? nowIso();
    const sourceLastInteractionAt =
      extractPromptField(prompt, "sourceLastInteractionAt") ?? updatedAt;
    return JSON.stringify({
      title: "Synthetic session notes",
      currentState: "",
      taskSpecification: "",
      filesAndFunctions: "",
      workflow: "",
      ...
      worklog: "",
      updatedAt,
      sourceLastInteractionAt,
    });
  }

  return "Synthetic one-off response.";
}
...
export const makeTestProviderAdapterHarness = (options?: MakeTestProviderAdapterHarnessOptions) =>
  Effect.gen(function* () {
    ...
    const runOneOffPrompt: ProviderAdapterShape<ProviderAdapterError>["runOneOffPrompt"] = (
      input,
    ) =>
      Effect.succeed({
        text: buildOneOffPromptResponse(input.prompt),
      });
