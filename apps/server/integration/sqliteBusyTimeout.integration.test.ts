...
it.each(["node", "bun"])("%s waits for a separate SQLite writer", async (runtime) => {
  ...
  try {
    await expect.poll(() => output.includes("ready") || child.exitCode !== null).toBe(true);
    ...
    await expect.poll(() => output.includes("writing") || child.exitCode !== null).toBe(true);
