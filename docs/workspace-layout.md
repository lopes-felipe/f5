...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@f5tools/shared/git`, `@f5tools/shared/DrainableWorker`) — no barrel index.
