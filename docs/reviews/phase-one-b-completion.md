...
- Checkpoints capture after terminal events, including aborted turns, rather than at intermediate diff events. During a running turn, its checkpoint diff stays pending; the completed diff includes the last edit. This intentionally changes the previous intermediate-diff behavior. Durable terminal session events remain the fallback. Running and interrupted state survives checkpoint publication in both database and live projections, including the web reducer.
...
- A missing worktree inside this profile's managed directory with a surviving branch is recreated before sending. Recreation checks for branch conflicts before changing registrations, removes only the requested stale registration, and never creates or moves a branch. Other missing worktrees retain their metadata. Paths outside the managed directory are not recreated. Startup preserves recoverable paths, and polling returns `worktreeMissing` without running Git in a missing directory. The queue reports missing refs or failed recreation as `worktree_missing`.
- Recreation takes keyed path and in-process repository locks, followed by a fail-fast SQLite lock shared by profiles through the real Git common directory. This introduces the coordinator needed for recovery. Phase 9 will extend it to setup, cleanup, terminal claims and file rewinds; those features are not part of this PR.
...
The initial full runs exposed an obsolete integration Git stub and a branch-only session refresh that erased a failed turn’s error state; both are corrected and the complete suite passes. The 5,000-file capture fixture remains within its 30-second Git deadline. No quantitative Phase 2 CPU improvement or resolution of the previously measured Phase 0 resource-limit failures is claimed. Online provenance and coverage validation passes after recording implementation commit `353cc8606`: 2,332 unique records, 1,836 interval commits, 496 legacy commits and zero pending reviews. All 33 Phase 1b records have final dispositions.
...
Recovery no longer prunes the repository. Regression tests preserve a second missing worktree's index and registration, retain the target registration when another checkout blocks recovery, and reject creation outside the profile's managed directory. Same-process operations wait on the Git common-directory semaphore before trying the cross-profile lock.

Nested-project checkpoint patches now use paths relative to the project directory. Missing-worktree polling displays “Worktree missing” instead of “Initialize Git” and keeps historical diff controls available. Startup clears missing paths without a branch or live project. A failed branch probe still preserves the path and logs a warning: an unavailable drive or transient Git error is not proof that the branch is gone.

CheckpointReactor now uses its injected Git service. Repeated diff updates skip only placeholder creation, leaving the handler's common processing intact. Progress without a stored task start is explicitly ignored and tested. Unexpected HTTP write errors log only a sanitized error code; expected disconnects stay quiet.

Settings reads retain the write semaphore. The cache holds secret references, and reads materialize them from the secret store. Removing the semaphore would expose partially written values before commit or rollback. The rollback regression now includes a reader started while uncommitted secret values are present.

Protocol version 2 remains appropriate: the client now consumes the missing-worktree state. No new protocol shape or version bump is introduced by this follow-up.

Follow-up validation on the same macOS arm64 environment: formatting, lint (nine existing warnings, zero errors), typecheck, the full workspace suite and all 131 extended real-Git tests passed. The server unit suite passed 2,461 tests; the browser suite passed 430 tests. Online validation still covers 2,332 ledger records with zero pending reviews. Focused recovery checks also reject relative paths, so Git cannot interpret a managed-looking path against a different working directory.
